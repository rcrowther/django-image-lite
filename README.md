...
## Overview
There is a base model called AbstractImage. From this, sub-models can be constructed. Each sub-model is an image repository. It tracks uploads in a database table, and stores files in a location. Locations are in subfolders of MEDIA)ROOT.

Each original image can generate derivative images. These are untracked and generated on a sucessful upload. These derivative images are called 'reforms' and are generated by filters. Reforms can be thumbnails, watermarked etc. 

Image delivery is by template tag. The tags write HTML elements with an appropriate URL. The model tag constructs the URL by asking the model instances they recieve.
...
### Make some filters
In the repository, create a flle image_filtetrs.py. Paste this into it,
...
Add this tag to the template,
    ...
    {% load img_tags %}
    ...
    {% image_fixed news_images.NewsImage riverbank-renforcement Thumb %}


'news_images.NewsImage' is the path to the repository model. 'riverbank-renforcement' is the filename. 'Thumb' is the filter we made earlier. 

Visit the page. You should see a thumbnail of the 'riverbank-renforcement' image.
...
It is impossible to have two files in the same directory with the same name (this applies also to remote storage). The default behaviour of Django uploading is to enable uploads if possible, so the standard FileSystemStorage module will rename suplicate filenames. It does this by adding a pseudo-random sequence of codepoints to the filename. 

For the general outlook of ImageLite, this is not always what is wanted. If a user has uploaded an image called 'Spin_X9000CA', a later attempt to add a  'Spin_X9000CA' image should be blocked, or replace the first image (standard behaviour would be to generate a new filename such as 'Spin_X9000_CA_WEN"£HDOE5'). 
...
### Base filters
The filter code is an in heriting stack of classes. There are some base filters, which you can configure. These are centre-anchored, 
...
However, especially for premium display of an assorment of images, it may be preferable that the image is always fully contained inside the declared dimensions. This is what the Smart filters do. Smart filters preserve aspect ratio, make the image fit in the dimensions, then fill surplus bounding areas with a fill colour,
...
== Uniqueness
 The usual Django solution for file fields is renaming. IF any file is repeated, then psudo-random collections of codepoints are appeneded, then repeatedly tried until the filename passes as unique.

However, in ImageLite it may be preferable to make file names unique. The user knows what they are doing, or is given that right, anyway. And the user does not want the filename automatically changing to garbage. So at least, we need to block file renaming.

I can think of a several ways to do this. Effectivly, it's always a validation action, but where?
- on form
Not possible to get at DB
...
- model level 
Readily get at the database
Hard to override
Could make an optional offering then?

https://docs.djangoproject.com/en/3.2/ref/models/instances/#validating-objects

Django does it's uniqueness checks as model validation. You'd think it was field level, but they must account for combined field unique constraints. In our case, it's tricky to get at the compromised field validation code, which is renaming through storage. And I don't know how to pass in a DB search anyhow.


== Unique
You may want filenames to be unique. The reason for this is a side effect, to prevent users from uploading the same file more than once, then finding the file is renamed.
...
=== Rationale
Django handles uniqueness within the model, not the field. This allows multi-field unique constraints. For best consistency, this code follows. We use a mixin because we assume they are simple. We assume this is a functionality many coders will wish to add.
